(function (root, factory) {
  if (typeof module === 'object' && module.exports) {
    module.exports = factory();
  } else {
    const library = factory();
    root.UPIKeywordBank = library;
    root.FRAUD_KEYWORDS = library.DEFAULT_KEYWORDS;
    root.FRAUD_KEYWORDS_EN = library.FRAUD_KEYWORDS_EN;
    root.FRAUD_KEYWORDS_HI = library.FRAUD_KEYWORDS_HI;
  }
})(typeof self !== 'undefined' ? self : this, function () {
  const FRAUD_KEYWORDS_EN = [
    { keyword: 'win', weight: 1, category: 'offer' },
    { keyword: 'winner', weight: 1, category: 'offer' },
    { keyword: 'winning', weight: 1, category: 'offer' },
    { keyword: 'lottery', weight: 3, category: 'offer', reason: 'Lottery bait is a common fraud lure.' },
    { keyword: 'jackpot', weight: 2, category: 'offer' },
    { keyword: 'free', weight: 1, category: 'offer' },
    { keyword: 'urgent', weight: 2, category: 'social-engineering' },
    { keyword: 'immediate action', weight: 3, category: 'social-engineering' },
    { keyword: 'click here', weight: 3, category: 'link' },
    { keyword: 'verify', weight: 2, category: 'credential' },
    { keyword: 'verification required', weight: 3, category: 'credential' },
    { keyword: 'otp', weight: 4, category: 'credential', reason: 'OTP requests are highly sensitive.' },
    { keyword: 'bank', weight: 1, category: 'authority-impersonation' },
    { keyword: 'account blocked', weight: 4, category: 'social-engineering' },
    { keyword: 'account suspended', weight: 4, category: 'social-engineering' },
    { keyword: 'account locked', weight: 4, category: 'social-engineering' },
    { keyword: 'prize', weight: 2, category: 'offer' },
    { keyword: 'reward', weight: 2, category: 'offer' },
    { keyword: 'claim now', weight: 3, category: 'social-engineering' },
    { keyword: 'limited offer', weight: 2, category: 'offer' },
    'update kyc',
    'kyc update',
    'suspended',
    'refund',
    'cashback',
    'link',
    'password',
    'login',
    'confirm',
    'gift',
    'transfer',
    'upi collect',
    'payment request',
    'security alert',
    'unauthorized',
    'risk',
    'call now',
    'support',
    'customer care',
    'fraud alert',
    'download app',
    'install app',
    'screen share',
    'remote access',
    'approval pending',
    'transaction failed',
    'resubmit',
    'beneficiary',
    'debit',
    'credit reversal',
    'chargeback',
    'pending refund',
    'expire today',
    'expires in',
    'act now',
    'pay now',
    'instant payout',
    'exclusive offer',
    'limited time',
    'extra income',
    'earn from home',
    'bonus credit',
    'register now',
    'verify account',
    'reactivate account',
    'unlock account',
    'secure your account',
    'maintain account',
    'mandatory update',
    'authentication failed',
    'identity validation',
    'identity proof',
    'kyc expired',
    'bank verification',
    'payment hold',
    'payment blocked',
    'auto debit',
    'auto pay',
    'collect request',
    'approve request',
    'request pending',
    'upi pin',
    'reset pin',
    'share pin',
    'share otp',
    'do not ignore',
    'final warning',
    'legal action',
    'penalty',
    'fine payment',
    'tax refund',
    'government refund',
    'wallet credit',
    'wallet blocked',
    'profile blocked',
    'document verification',
    'aadhaar update',
    'pan verification',
    'phone verification',
    'suspicious activity',
    'new device login',
    'unusual login',
    'your account is at risk',
    'safety check',
    'safety verification',
    'security check',
    'verify immediately',
    'validate now',
    'send details',
    'share details',
    'enter details',
    'confirm identity',
    'card blocked',
    'card update',
    'card verification',
    'debit card',
    'credit card',
    'cvv',
    'expiry date',
    'atm pin',
    'bank executive',
    'relationship manager',
    'helpline',
    '24x7 support',
    'raise complaint',
    'complaint closure',
    'priority customer',
    'premium user',
    'selected user',
    'pre approved',
    'loan approved',
    'loan disbursal',
    'processing fee',
    'application fee',
    'registration fee',
    'service charge',
    'verification charge',
    'fund release',
    'clearance fee',
    'refund fee',
    'security deposit',
    'advance payment',
    'full refund guaranteed',
    'guaranteed return',
    'double your money',
    'investment plan',
    'high return',
    'crypto payout',
    'bitcoin reward',
    'airdrop',
    'trading account',
    'dm for details',
    'whatsapp support',
    'telegram support',
    'chat now',
    'contact support',
    'contact immediately',
    'click to verify',
    'tap to claim',
    'open this link',
    'secure link',
    'verification link',
    'shorturl',
    'bit.ly',
    'tinyurl',
    'http',
    'https',
    'unknown sender',
    'not your transaction',
    'if not you',
    'stop transaction',
    'cancel payment',
    'cancel now',
    'block account',
    'freeze account',
    'recover amount',
    'recovery team',
    'final attempt',
    'last chance',
    'claim reward',
    'redeem reward',
    'special reward',
    'festival offer',
    'cash prize',
    'big prize',
    'surprise reward',
    'selected for reward',
    'congratulations',
    'congrats',
    'dear customer',
    'dear user',
    'trusted partner',
    'official message',
    'official update',
    'important notice',
    'important update',
    'action required',
    'verify in 10 minutes',
    'within 30 minutes',
    'respond immediately',
    'urgent response',
    'account will close',
    'service termination',
    'service suspended',
    'payment suspended',
    'reactivate service',
    'unblock service',
    'pin expired',
    'session expired',
    'login expired',
    're login',
    'mobile number verification',
    'sim blocked',
    'sim upgrade',
    'network issue',
    'technical issue',
    'temporary hold',
    'amount credited',
    'amount debited',
    'failed credit',
    'pending credit',
    'pending debit',
    'reverse transaction',
    'merchant verification',
    'seller verification',
    'buyer protection',
    'safe payment',
    'insurance refund',
    'policy refund',
    'utility cashback',
    'electricity bill refund',
    'gas subsidy',
    'subsidy credit',
    'claim subsidy',
    'support ticket',
    'ticket closed',
    'ticket id',
    'verify ticket',
    'payment gateway',
    'gateway verification',
    'collect mandate',
    'autopay mandate',
    'mandate approval',
    'request money',
    'receive money',
    'scan qr',
    'qr payment',
    'confirm collect',
    'upi handle',
    'vpa',
    'beneficiary added',
    'new payee',
    'new beneficiary',
    'device binding',
    'account migration',
    'migration required',
    'claim compensation',
    'compensation credit',
    'instant compensation'
  ];

  const FRAUD_KEYWORDS_HI = [
    { keyword: 'ओटीपी', weight: 4, category: 'credential', reason: 'OTP requests are highly sensitive.' },
    { keyword: 'otp शेयर', weight: 4, category: 'credential' },
    { keyword: 'पिन', weight: 4, category: 'credential' },
    { keyword: 'यूपीआई पिन', weight: 4, category: 'credential' },
    { keyword: 'पिन साझा करें', weight: 4, category: 'credential' },
    { keyword: 'पासवर्ड', weight: 3, category: 'credential' },
    { keyword: 'लॉगिन', weight: 2, category: 'credential' },
    { keyword: 'वेरिफाई', weight: 2, category: 'credential' },
    { keyword: 'सत्यापन आवश्यक', weight: 3, category: 'credential' },
    { keyword: 'केवाईसी', weight: 3, category: 'credential' },
    { keyword: 'केवाईसी अपडेट', weight: 3, category: 'credential' },
    { keyword: 'खाता ब्लॉक', weight: 4, category: 'social-engineering' },
    { keyword: 'खाता निलंबित', weight: 4, category: 'social-engineering' },
    { keyword: 'खाता बंद', weight: 4, category: 'social-engineering' },
    { keyword: 'तुरंत', weight: 2, category: 'social-engineering' },
    { keyword: 'अभी कार्रवाई', weight: 3, category: 'social-engineering' },
    { keyword: 'अंतिम चेतावनी', weight: 4, category: 'social-engineering' },
    { keyword: 'इनाम', weight: 2, category: 'offer' },
    { keyword: 'लॉटरी', weight: 3, category: 'offer' },
    { keyword: 'बधाई हो', weight: 2, category: 'offer' },
    { keyword: 'क्लिक करें', weight: 3, category: 'link' },
    { keyword: 'लिंक', weight: 2, category: 'link' },
    { keyword: 'भुगतान अनुरोध', weight: 3, category: 'payment' },
    { keyword: 'कलेक्ट रिक्वेस्ट', weight: 3, category: 'payment' },
    { keyword: 'रिफंड', weight: 2, category: 'payment' },
    { keyword: 'कैशबैक', weight: 2, category: 'offer' },
    { keyword: 'सपोर्ट नंबर', weight: 2, category: 'authority-impersonation' },
    { keyword: 'कस्टमर केयर', weight: 2, category: 'authority-impersonation' },
    { keyword: 'फ्रॉड अलर्ट', weight: 3, category: 'authority-impersonation' },
    { keyword: 'सरकारी रिफंड', weight: 3, category: 'authority-impersonation' }
  ];

  const DEFAULT_KEYWORDS = FRAUD_KEYWORDS_EN.concat(FRAUD_KEYWORDS_HI);

  return {
    FRAUD_KEYWORDS_EN,
    FRAUD_KEYWORDS_HI,
    DEFAULT_KEYWORDS
  };
});
