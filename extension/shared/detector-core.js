(function (root, factory) {
  ...
  function analyzeMessage(message, keywords, options) {
    const cleanMessage = String(message || '');
    const normalizedMessage = cleanMessage.toLowerCase();
    const settings = options || {};
    const includeHighlight = settings.includeHighlight !== false;
    ...
    const charFlags = includeHighlight ? new Array(cleanMessage.length).fill(false) : [];
    if (includeHighlight) {
    ...
    return {
      highlightedHtml: includeHighlight ? buildHighlightedText(cleanMessage, charFlags) : '',
      detectedKeywords,
      totalMatches,
      riskScore,
